...
def get_input():
    ...
    env_vars = os.environ
    if 'INPUT_FILENAME' not in env_vars:
        throw_invalid_env('INPUT_FILENAME')

    # Validate that the input file exists.
    input_filename = env_vars['INPUT_FILENAME']
    if not os.path.isfile(input_filename):
        throw_not_found(input_filename, 'INPUT_FILENAME')
...
def get_output():
    ...
    env_vars = os.environ
    if 'OUTPUT_FILENAME' not in env_vars:
        throw_invalid_env('OUTPUT_FILENAME')

    # Check whether the output file exists.
    output_filename = env_vars['OUTPUT_FILENAME']
